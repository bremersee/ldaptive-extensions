...
package org.bremersee.ldap.security.userdetails;

import static org.bremersee.ldap.LdaptiveEntryMapper.getAttributeValue;
import static org.bremersee.ldap.LdaptiveEntryMapper.getAttributeValuesAsSet;
import static org.bremersee.ldap.transcoder.UserAccountControlValueTranscoder.isUserAccountEnabled;
...
import org.bremersee.ldap.LdaptiveEntryMapper;
import org.bremersee.ldap.transcoder.UserAccountControlValueTranscoder;
...
public class UserDetailsLdapMapper implements LdaptiveEntryMapper<UserDetails> {
  ...
  public UserDetailsLdapMapper(
    ...
    this.authorityTranscoder = new GrantedAuthorityValueTranscoder(authorityDn, authorityMap,
        authorityPrefix);
  ...
  @Override
  public AttributeModification[] mapAndComputeModifications(UserDetails source,
  ...
  protected boolean isAccountEnabled(LdapEntry ldapEntry) {
    return !StringUtils.hasText(getUserAccountControlAttributeName()) || isUserAccountEnabled(
        getAttributeValue(
            ldapEntry, getUserAccountControlAttributeName(), getUserAccountControlValueTranscoder(),
            null));
  }
  ...
  protected Collection<? extends GrantedAuthority> getGrantedAuthorities(LdapEntry ldapEntry) {
    ...
    if (StringUtils.hasText(getAuthorityAttributeName())) {
      grantedAuthorities.addAll(
          getAttributeValuesAsSet(ldapEntry, getAuthorityAttributeName(),
              getAuthorityTranscoder()));
    }
   ...
   */
  @ToString
  protected static class GrantedAuthorityValueTranscoder extends
      AbstractStringValueTranscoder<GrantedAuthority> {
    ...
    public GrantedAuthority decodeStringValue(String value) {
      ...
      String authorityValue = UserDetailsLdapMapper.prefixAuthority(getAuthorityPrefix(),
          mappedValue);
    ...
    public String encodeStringValue(GrantedAuthority value) {
      throw new UnsupportedOperationException(
          "Getting ldap attribute value from granted authority is not supported.");
    }
